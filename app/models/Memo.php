...
class Memo extends BaseModel {
    ...
    public static function all($kayt_id) {
        $query = DB::connection()->prepare('SELECT * FROM Muistiinpano WHERE kayt_id = :kayt_id ORDER BY priority DESC, added ASC');
    ...
    public static function all_groupmemos($attr) {
        $query = DB::connection()->prepare('SELECT RyhmaMuistiinpano.id, Ryhmamuistiinpano.kayt_id, Ryhmamuistiinpano.name,'
                . 'Ryhmamuistiinpano.description, Ryhmamuistiinpano.added, Ryhmamuistiinpano.priority, ryhma.name as groupname FROM ryhma, RyhmaMuistiinpano, Kayttaja, Kayt_ryhma WHERE kayttaja.id = :attr AND ryhma.id = kayt_ryhma.ryhma_id AND kayttaja.id = kayt_ryhma.kayt_id AND RyhmaMuistiinpano.ryh_id = kayt_ryhma.ryhma_id ORDER BY ryhmaMuistiinpano.priority DESC, ryhmaMuistiinpano.added ASC;');
...
//  Kint::dump($row);
    ...
    public function validate_description() {
        $errors = array();
        $errors = array_merge($errors, BaseModel::validate_String_not_null($this->description, 'Kuvauksen'));
        $errors = array_merge($errors, BaseModel::validate_String_lenght($this->description, 'Kuvauksen'));
        $errors = array_merge($errors, BaseModel::validate_not_whitespace($this->description, 'Kuvauksen'));
        return $errors;
    }

    public function validate_priority() {
        $errors = array();
        $errors = array_merge($errors, BaseModel::validate_String_not_null($this->priority, 'Tärkeyden'));
        $errors = array_merge($errors, BaseModel::validate_not_whitespace($this->priority, 'Tärkeyden'));
        $errors = array_merge($errors, BaseModel::validate_integer($this->priority, 'Tärkeyden'));
        return $errors;
    }
